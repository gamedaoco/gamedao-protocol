...
#[scale_info(skip_type_params(MaxVotesPerProposal))]
#[cfg_attr(feature = "std", derive(Debug))]
pub struct Voting<AccountId, Balance, MaxVotesPerProposal>
where
MaxVotesPerProposal: Get<u32>,
{
	pub index: ProposalIndex, // Nonce
	pub unit: Unit, // Person or Token
	// 1. Voting process:
	// Currently support only Yes/No voting type
	pub ayes: BoundedVec<(AccountId, VotingPower, Option<Balance>), MaxVotesPerProposal>,
	pub nays: BoundedVec<(AccountId, VotingPower, Option<Balance>), MaxVotesPerProposal>,
	// For multiple choice/options should be refactored, ex.:
	// 	pub votes: BoundedVec<(AccountId, Option, VotingPower, Option<Balance>), MaxVotesPerProposal>,
