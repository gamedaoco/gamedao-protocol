...
use frame_support::{
	...
	dispatch::result, ensure, traits::{Currency, ReservableCurrency}
};
...
impl<T:Trait> Module<T>{
	fn can_use_balance(request_id: T::Hash, supported_count: u64) -> Result{
		...
		ensure!(remain_balance >= request_balance, Error::<T>::BalanceTooLow);
		// Get the owner of the funding
		let owner = <RequestOwner<T>>::get(&request_id).ok_or(Error::<T>::NoOwnerFound)?;
