...
decl_storage! {
	trait Store for Module<T: Trait> as FundingFactory {
		// Global state
		Fundings get(fn funding_by_id):
			map hasher(blake2_128_concat) AccountId => Funding<T::Hash, T::AccountId, T::Balance, T::BlockNumber>;
		// Owner of a funding project
		FundingOwner get(fn owner_of):
			map hasher(blake2_128_concat) AccountId => Option<T::AccountId>;
		// Maximum time limit for the project
		FundingPeriodLimit get(fn funding_period_limit) config():
			T::BlockNumber = T::BlockNumber::sa(777600);

		// Projects ending in a block
		FundingsByBlockNumber get(fn funding_expire_at):
			map hasher(blake2_128_concat) T::BlockNumber => Vec<T::Hash>;

		// All funding state
		AllFundingArray get(fn funding_by_index):
			map hasher(blake2_128_concat) u64 => T::Hash;
		AllFundingCount get(fn all_funding_count):
			u64;
		AllFundingIndex:
			map hasher(blake2_128_concat) T::Hash => u64;

		// The user's funding state
		OwnedFundingArray get(fn funding_of_owner_by_index):
			map hasher(blake2_128_concat) (T::AccountId, u64) => T::Hash;
		OwnedFundingCount get(fn owned_funding_count):
			map hasher(blake2_128_concat) T::AccountId => u64;
		OwnedFundingIndex:
			map hasher(blake2_128_concat) (T::AccountId, T::Hash) => u64;

		// The investor's funding state
		InvestedFundingsArray get(fn invested_funding_by_index):
			map hasher(blake2_128_concat) (T::AccountId, u64) => T::Hash;
		InvestedFundingsCount get(fn invested_funding_count):
			map hasher(blake2_128_concat) T::AccountId => u64;
		InvestedFundingsIndex:
			map hasher(blake2_128_concat) (T::AccountId, T::Hash) => u64;

		// The investor invested how much money for a project
		InvestAmount get(fn invest_amount_of):
			map hasher(blake2_128_concat) (T::Hash, T::AccountId) => T::Balance;
		// Investors that who had invested the project before
		InvestAccounts get(fn invest_accounts):
			map hasher(blake2_128_concat) T::Hash => Vec<T::AccountId>;
		InvestAccountsCount get(fn invest_accounts_count):
			map hasher(blake2_128_concat) T::Hash => u64;

		// The total amount of money the project has got
		FundingSupportedAmount get(fn total_amount_of_funding):
			map hasher(blake2_128_concat) T::Hash => T::Balance;
...
decl_error! {
	pub enum Error for Module<T: Trait> {
		/// A GenericError occured.
		GenericError,
		/// Overflow adding a new funding to total fundings
		OverflowAddFunding,
		/// Overflow adding a new funding to account balance
		OverflowAddToAccountBalance,
	}
}
...
impl<T: Trait> Module<T> {
	...
	fn mint(sender: T::AccountId, funding_id: T::Hash, expiry: T::BlockNumber, support_money: T::Balance, new_funding: Funding<T::Hash, T::AccountId, T::Balance, T::BlockNumber>) -> Result{
		...
		let new_all_funding_count = all_funding_count.checked_add(1).ok_or(Error::<T>::OverflowAddToTotalFunding)?;

		let owned_funding_count = Self::owned_funding_count(&sender);
		let new_owned_funding_count = owned_funding_count.checked_add(1).ok_or(Error::<T>::OverflowAddToAccountBalance)?;
