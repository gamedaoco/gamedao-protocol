  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card>
          <CardHeader>
            <CardTitle>Total Collectives</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{stats.totalOrganizations}</div>
            <p className="text-muted-foreground text-sm">
              All collectives in the ecosystem
            </p>
          </CardContent>
        </Card>
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          <EmptyState
            type="organizations"
            title={filter === 'all' ? 'No collectives found' : `No ${filter} collectives found`}
            description={
              filter === 'all'
                ? 'Get started by creating your first gaming DAO or discover existing communities.'
                : `There are currently no ${filter} collectives. Try changing your filter.`
            }
            primaryAction={{
        ...
        {filteredOrganizations.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredOrganizations.map((org) => {
              const metaResult = org.metadataURI ? metadataResults[org.metadataURI] : null
              const bannerImage = metaResult?.bannerImageUrl || undefined
              const profileImage = metaResult?.profileImageUrl || undefined

              // optional debug: console.log('image urls', { org: org.id, bannerImage, profileImage })
              ...
              return (
                <EntityCard
                  ...
                  id={org.id}
                  title={org.name}
                  description={`${getAccessModelString(org.accessModel)} collective with ${org.memberCount} members`}
                  banner={bannerImage}
                  icon={profileImage ? <img src={profileImage} alt="" className="h-5 w-5 rounded" /> : <Users className="h-5 w-5" />}
