...
/**
 * @title Registry
 * @dev Central registry for all GameDAO modules
 * @author GameDAO AG
 */
contract Registry is IRegistry, AccessControl, Pausable, ReentrancyGuard {
    ...
    // Events (others inherited from IRegistry)
        ...
        nonReentrant
    {
        ...
        // Notify the module
        IModule(moduleInfo.moduleAddress).onModuleEnabled();
        ...
        nonReentrant
    {
        ...
        // Notify the module
        IModule(moduleInfo.moduleAddress).onModuleDisabled();
        ...
        nonReentrant
    {
        ...
        // Get new module info
        IModule newModule = IModule(newModuleAddress);
        ...
        if (wasEnabled) {
            moduleInfo.isEnabled = false;
            _enabledModules.remove(moduleId);
            IModule(oldAddress).onModuleDisabled();
        }
    ...
    function emergencyDisableAllModules() external onlyRole(ADMIN_ROLE) {
        ...
        for (uint256 i = 0; i < enabledModules.length; i++) {
            ...
            // Notify the module
            try IModule(moduleInfo.moduleAddress).onModuleDisabled() {
