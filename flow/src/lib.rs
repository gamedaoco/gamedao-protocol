...
use frame_support::{codec::{Decode, Encode}, dispatch::DispatchResult, traits::{UnixTime, Randomness}};
...
use primitives::{Balance, Moment, CurrencyId};
...
pub mod pallet {
	...
	pub(super) type Campaigns<T: Config> = StorageMap<_, Blake2_128Concat, T::Hash, Campaign<T::Hash, T::AccountId, Balance, T::BlockNumber, Moment, FlowProtocol, FlowGovernance>, ValueQuery>;
	...
	pub(super) type CampaignState<T: Config> = StorageMap<_, Blake2_128Concat, T::Hash, FlowState, ValueQuery, GetDefault>;
	...
	pub(super) type CampaignsByBlock<T: Config> = StorageMap<_, Blake2_128Concat, T::BlockNumber, Vec<T::Hash>, ValueQuery>;
	...
	#[pallet::storage]
	pub(super) type CampaignsOwnedIndex<T: Config> = StorageMap<_, Blake2_128Concat, (T::Hash, T::Hash), u64, ValueQuery>;

	/// campaigns contributed by accountid
	#[pallet::storage]
	#[pallet::getter(fn campaigns_contributed)]
	pub(super) type CampaignsContributed<T: Config> = StorageMap<_, Blake2_128Concat, T::AccountId, Vec<T::Hash>, ValueQuery>;

	/// campaigns related to an organisation
	#[pallet::storage]
	#[pallet::getter(fn campaigns_by_body)]
	pub(super) type CampaignsByBody<T: Config> = StorageMap<_, Blake2_128Concat, T::Hash, Vec<T::Hash>, ValueQuery>;

	// caller contributed campaigns -> contributed campaigns
	#[pallet::storage]
	#[pallet::getter(fn campaigns_contributed_index)]
	pub(super) type CampaignsContributedArray<T: Config> = StorageMap<_, Blake2_128Concat, (T::AccountId, u64), T::Hash, ValueQuery>;
	#[pallet::storage]
	#[pallet::getter(fn campaigns_contributed_count)]
	pub(super) type CampaignsContributedCount<T: Config> = StorageMap<_, Blake2_128Concat, T::AccountId, u64, ValueQuery>;
	#[pallet::storage]
	pub(super) type CampaignsContributedIndex<T: Config> = StorageMap<_, Blake2_128Concat, (T::AccountId, T::Hash), u64, ValueQuery>;

	// Total contributions balance per campaign
	#[pallet::storage]
	#[pallet::getter(fn campaign_balance)]
	pub(super) type CampaignBalance<T: Config> = StorageMap<_, Blake2_128Concat, T::Hash, Balance, ValueQuery>;

	// Contributions per user
	#[pallet::storage]
	#[pallet::getter(fn campaign_contribution)]
	pub(super) type CampaignContribution<T: Config> = StorageMap<_, Blake2_128Concat, (T::Hash, T::AccountId), Balance, ValueQuery>;

	// Contributors
	#[pallet::storage]
	#[pallet::getter(fn campaign_contributors)]
	pub(super) type CampaignContributors<T: Config> = StorageMap<_, Blake2_128Concat, T::Hash, Vec<T::AccountId>, ValueQuery>;
	#[pallet::storage]
    ...
    pub enum Event<T: Config> {
        ...
        CampaignCreated(T::Hash, T::AccountId, T::AccountId, Balance, Balance, T::BlockNumber, Vec<u8>),
		...
		fn on_finalize(_n: BlockNumberFor<T>) {
			...
			for campaign_id in &campaign_hashes {
				...
				// campaign cap not reached
					...
					let unreserve_deposit = T::Currency::unreserve(T::FundingCurrencyId::get(), &dao_treasury, campaign.deposit);
	...
	impl<T: Config> Pallet<T> {
        ...
        pub fn create(
			...
			ensure!((contributions.len() as u32) < T::MaxCampaignsPerBlock::get(), Error::<T>::ContributionsPerBlockExceeded );
        ...
        pub fn contribute (
			...
			let sender = ensure_signed(origin)?;
			ensure!( T::Currency::free_balance(T::FundingCurrencyId::get(), &sender) >= contribution, Error::<T>::BalanceTooLow );
			let owner = Self::campaign_owner(campaign_id) .ok_or(Error::<T>::OwnerUnknown)?;
			...
			let campaign = Self::campaign_by_id(&campaign_id);
			ensure!(<frame_system::Pallet<T>>::block_number() < campaign.expiry, Error::<T>::CampaignExpired );
...
impl<T: Config> Pallet<T> {
	...
	// campaign object
	pub fn mint(
		campaign: Campaign<T::Hash, T::AccountId, Balance, T::BlockNumber, Moment, FlowProtocol, FlowGovernance>
	) -> DispatchResult {
		...
		T::Currency::reserve(T::FundingCurrencyId::get(), &treasury, campaign.deposit.clone())?;
